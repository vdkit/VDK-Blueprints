---
id: integrate
name: System Integration Builder
description: >-
  Connect services, APIs, databases, and tools to create seamless integrations
...
examples:
  - usage: /integrate api with database
    description: >-
      Create REST API integration with database including caching and error
      handling
    context: >-
      Building microservice that needs to sync data between external API and
      local database
    expectedOutcome: >-
      API client, database layer, caching, retry logic, and monitoring
      integration
  - usage: /integrate webhook with queue
    description: Set up webhook receiver that publishes events to message queue
    context: >-
      Processing third-party webhooks asynchronously with reliable message
      handling
    expectedOutcome: >-
      Webhook endpoint, message queue publisher, signature verification, and
      event processing
  - usage: /integrate oauth with service
    description: Implement OAuth authentication flow with external service provider
    context: Adding social login functionality with Google/GitHub authentication
    expectedOutcome: 'OAuth flow, token management, user profile sync, and session handling'
installation:
  dependencies:
    - deno
    ...
    - monitoring tools
  setupSteps:
    - Install required SDKs and client libraries
...
compatibilityNotes: >-
  Supports REST APIs, databases (PostgreSQL, Redis), message queues (RabbitMQ),
  and OAuth providers
schemaVersion: '3.0'
title: System Integration Builder
kind: command
specificityLayer: L2
platforms:
  claude-code:
